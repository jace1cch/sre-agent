...
from sre_agent.deployment.readiness import DeploymentCheck
...
def test_readiness_report_warns_without_linux_sources(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "sre_agent.deployment.readiness._check_linux_platform",
        lambda: DeploymentCheck(
            name="linux_platform",
            status="fail",
            summary="This deployment target is not Linux with /proc support.",
        ),
    )
